...
# 1. Installl with install.packages(), type = "binary"
...
setwd("/Users/ayazhan/Desktop/Projects/Movie-Recommendation-System-using-R_Project-main/IMDB-Dataset")
...
summary(movie_data) # movie, title, genres
head(movie_data)
summary(rating_data) # userID, movieId, rating, timestamp
head(rating_data)
```
...
SearchMatrix <- cbind(movie_data[,1:2], genre_mat2[]) # combine specified Vector, Matrix or Data Frame by columns
head(SearchMatrix)
...
for (index in 1:10325){table_views[index,3] <- as.character(subset(movie_data,movie_data$movieId == table_views[index,1])$title)}
table_views[1:6,]
```
```{r}
